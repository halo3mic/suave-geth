...
type SuaveRuntime interface {
	...
	doHTTPRequests(requests []types.HttpRequest) ([][]byte, error)
...
func (b *SuaveRuntimeAdapter) doHTTPRequests(input []byte) (res []byte, err error) {
	var (
		unpacked []interface{}
		result   []byte
	)
	...
	unpacked, err = artifacts.SuaveAbi.Methods["doHTTPRequests"].Inputs.Unpack(input)
	if err != nil {
		err = errFailedToUnpackInput
		return
	}
	...
	if err = mapstructure.Decode(unpacked[0], &requests); err != nil {
		err = errFailedToDecodeField
		return
	}
	...
	if httpResponses, err = b.impl.doHTTPRequests(requests); err != nil {
		return
	}

	result, err = artifacts.SuaveAbi.Methods["doHTTPRequests"].Outputs.Pack(httpResponses)
	if err != nil {
		err = errFailedToPackOutput
		return
	}
